...
<template>
  <header class="bg-gray-700">
    <nav class="container mx-auto p-4 flex justify-between text-white">
      ...
      <NuxtLink v-show="auth.user" class="hover:text-amber-500" to="/manage">
        Manage
      </NuxtLink>
      <button v-show="!auth.user" class="hover:text-amber-500" @click="modal.showModal()">
        Login / Register
      </button>
      <button v-show="auth.user" class="hover:text-amber-500" @click="logout">
        Sign Out
      </button>
    </nav>
  </header>
